...
## [2026-06-16 08:02] - Optimización de CPU, Tracking HD en Movimiento y Robustecimiento de Blurring

**Archivos modificados:** `ai/src/configuracion.py`, `ai/src/detector.py`, `ai/src/app.py`, `ai/src/streamer.py`

**Resumen Técnico:**
1. **Re-empaquetamiento del Modelo YOLO (`best.pt`):** Se identificó que la actualización de pesos enviada por el equipo de IA se cargó como una estructura de carpeta descomprimida en la raíz del proyecto. Se diseñó un script de compresión multiplataforma en Python usando `zipfile` para empaquetarla de nuevo en un fichero `.pt` válido para PyTorch y Ultralytics. Se estructuró con un subdirectorio contenedor raíz (`best/`) y rutas con barras diagonales `/` para garantizar la compatibilidad del formato, validando su carga exitosa en el motor de inferencia.
2. **Tracking HD en Movimiento (Resolución del Agrupamiento):** Al elevar la resolución del sensor de captura a 720p (1280x720) en la sesión anterior, la distancia horizontal en píxeles entre las prendas de una persona en movimiento superaba el umbral rígido original de `160px`. Esto causaba falsos negativos de acceso por "falta de pantalón". Se agregó `umbral_agrupamiento_px: int = 320` en `ConfiguracionIA` y se parametrizaron `DetectorVestimenta` y `EstabilizadorVentana` para usar este umbral flexible, estabilizando las alertas y el suavizado temporal al simular caminata o giros laterales.
3. **Censura de Rostros Persistente y Expandida:**
   * **Margen de seguridad:** En `CensorPrivacidad.aplicar_desenfoque`, se expandió simétricamente el área de blur en un `15%` de su ancho y alto para evitar brechas de visualización del rostro al desplazarse de forma rápida entre los frames asíncronos.
   * **Persistencia temporal (Inercia):** Se programó en el detector de rostros un acumulador de inercia que mantiene las regiones de cara de la iteración previa hasta por 3 ciclos de inferencia (~18 frames o 600ms) si Haar Cascades reporta 0 rostros momentáneamente debido a sombras o ángulos.
   * **Sensibilidad:** Se redujo `vecinos_minimos_rostro` a `3` en `configuracion.py` para facilitar la detección de múltiples personas y rostros con leves giros.
4. **Streaming de Video bajo Demanda (Ahorro de CPU):** En `streamer.py`, se implementó un control de conexiones activas (`clientes_activos` con exclusión mutua Lock). El motor de streaming MJPEG detecta de forma atómica si hay navegadores consumiendo el stream `/stream`. Si no hay clientes activos (el portero no está usando "Ver en vivo"), se omite por completo la costosa operación matemática `cv2.imencode` a JPEG a 30 FPS de la imagen de 1280x720, reduciendo el consumo de CPU a un porcentaje mínimo en segundo plano.
5. **Aligeramiento de Detección de Rostros:** Se disminuyó `escala_redimension_censor` a `0.40` (antes `0.60`), reduciendo el número de píxeles a escanear por Haar Cascades en un 55% sin afectar la precisión para el rango de 1.0m - 2.0m de distancia.
