...
class MotorVisionIA:
    ...
    def _gestionar_alertas(
        ...
        if not resultado.cumple_normativa:
            ...
            log.info(
                "ALERTA: Incumplimiento | Faltantes: %s",
                self._obtener_resumen_faltantes(resultado),
            )
        else:
    ...
    def _dibujar_hud(
        ...
        try:
            ...
            ancho_panel, alto_panel = 210, 185
            px1, py1 = 15, 15
            px2, py2 = px1 + ancho_panel, py1 + alto_panel

            overlay = frame_hud.copy()
            cv2.rectangle(overlay, (px1, py1), (px2, py2), (14, 14, 14), -1)
            cv2.addWeighted(overlay, 0.75, frame_hud, 0.25, 0, frame_hud)

            color_b = (45, 160, 45) if resultado.cumple_normativa else (30, 30, 200)
            cv2.rectangle(frame_hud, (px1, py1), (px2, py1 + 35), color_b, -1)
            msg = " ACCESO CONCEDIDO" if resultado.cumple_normativa else " ACCESO DENEGADO"
            cv2.putText(
                frame_hud, msg, (px1 + 5, py1 + 25),
                cv2.FONT_HERSHEY_DUPLEX, 0.55, (255, 255, 255), 1,
            )

            items = [
                ("Carnet",   resultado.carnet),
                ("Chaqueta", resultado.chaqueta_unefa),
                ("Pantalon", resultado.pantalon_oscuro),
                ("Chemise",  resultado.uniforme_superior),
            ]

            y_pos = py1 + 65
            for nombre, estado in items:
                txt = f"[{'OK' if estado else ' X '}] {nombre}"
                col = (120, 255, 120) if estado else (100, 100, 255)
                cv2.putText(
                    frame_hud, txt, (px1 + 15, y_pos),
                    cv2.FONT_HERSHEY_DUPLEX, 0.5, col, 1,
                )
                y_pos += 28
